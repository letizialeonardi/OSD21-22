...
{ 	
        ...
        printf ("%d: %s\n", j, mess);
        j++;
/* NOTA BENE: delle due close non effettuate (quella nel codice del figlio e quella nel codice del padre) e' quella mancante nel codice del padre che produce un blocco del padre, dato che il sistema pensa che ci sia un processo che puo' scrivere sulla pipe e quindi la read da pipe NON torna mai zero e quindi il padre rimane bloccato sulla read in attesa del verificarsi di una condizione che non si puo' verificare dato che l'unico processo che scrive sulla pipe (il figlio) ad un certo punto termina! */
}
